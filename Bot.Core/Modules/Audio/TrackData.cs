...
namespace Stormbot.Bot.Core.Modules.Audio
{
    ...
    internal class TrackData
    {
        ...
        private TrackData(string location)
        {
            Location = location;
        }

        private TrackData(string location, string name) : this(location)
        {
            Name = name;
        }
        ...
        internal static TrackData CreateRaw(string streamUrl) => new TrackData(streamUrl, $"Raw stream {streamUrl}") { IsRaw = true };

        internal static TrackData Parse(string location)
        {
